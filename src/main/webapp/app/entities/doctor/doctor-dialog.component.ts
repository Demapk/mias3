...
import { Speciality, SpecialityService } from '../speciality';
import { HealthFacility, HealthFacilityService } from '../health-facility';
...
export class DoctorDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.specialityService.query()
            .subscribe((res: HttpResponse<Speciality[]>) => { this.specialities = res.body; }, (res: HttpErrorResponse) => this.onError(res.message));
        this.healthFacilityService.query()
            .subscribe((res: HttpResponse<HealthFacility[]>) => { this.healthfacilities = res.body; }, (res: HttpErrorResponse) => this.onError(res.message));
    }
    ...
    trackSpecialityById(index: number, item: Speciality) {
        return item.id;
    }

    trackHealthFacilityById(index: number, item: HealthFacility) {
        return item.id;
    }
}
